...
### Fixed
- Header nav (About / Projects / Thoughts) and the mobile hamburger are now vertically centred against the site-title block. Causes: Skeleton's floated `.row` / `.columns` top-align the shorter nav on the taller title; Skeleton's `li { margin-bottom: 1rem }` added 10px under each header-nav item; on mobile/tablet Skeleton's `button { height: 38px; margin-bottom: 1rem }` sat the hamburger 4.37px below the title centre (same row — the live `initial-scale=0.7` viewport keeps a single-line title). `.navigation-cover` is now `display: flex; align-items: center` (row clearfix disabled so it is not a flex item), `.navigation-cover .navigation li` drops that bottom margin (overlay, footer, and content lists keep Skeleton's `li` spacing), and the hamburger overrides Skeleton's button height/margin
