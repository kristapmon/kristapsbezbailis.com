...
### Fixed
- Header nav (About / Projects / Thoughts) and the mobile hamburger are now vertically centred against the site-title block. Skeleton's floated `.row` / `.columns` top-aligned the shorter nav on the taller title; `.navigation-cover` is now `display: flex; align-items: center` (the row clearfix is disabled so it is not a flex item). On ≤925px the unused desktop nav column is hidden and `.mobile-menu` uses `box-sizing: border-box` so its padding stays inside the 20% width and the hamburger stays on the title row
