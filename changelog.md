...
### Fixed
- Header nav (About / Projects / Thoughts) and the mobile hamburger are now vertically centred against the site-title block. Causes: Skeleton's floated `.row` / `.columns` top-align the shorter nav on the taller title; Skeleton's `li { margin-bottom: 1rem }` added 10px under each nav item; on mobile, Skeleton's `button { height: 38px; margin-bottom: 1rem }` and content-box padding on `.mobile-menu` dropped the hamburger onto the next row. `.navigation-cover` is now `display: flex; align-items: center` (row clearfix disabled so it is not a flex item), `.navigation li` drops that bottom margin, and the hamburger overrides Skeleton's button height/margin. On ≤925px the unused desktop nav column is hidden and `.mobile-menu` uses `box-sizing: border-box` so it stays on the title row
